...
import { MagicWand as MagicWandIcon } from '@phosphor-icons/react/dist/ssr/MagicWand'; // This is a placeholder, replace with your chosen icon
...
export function TextEditorToolbar({ editor }: TextEditorToolbarProps): React.JSX.Element {
  ...
  const [isLoading, setLoading] = React.useState(false); // State to track loading
  ...
  const handleSendToAPI = async () => {
    ...
    try {
      const response = await fetch('https://resumify-backend.onrender.com/api/ai/enhance', {
        method: 'POST',
        headers: {
      ...
      >
        {editor ? (
          <Stack direction="row" spacing={0.5} sx={{ alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <Tooltip title="Enhance with AI 🧠" enterDelay={500} leaveDelay={200}>
              <span>
                <ToolbarButton
                ...
                >
                  {isLoading ? <CircularProgress size={24} /> : <MagicWandIcon />}
                </ToolbarButton>
              </span>
            </Tooltip>
          </Stack>
...
const ToolbarButton = React.forwardRef<HTMLButtonElement, ToolbarButtonProps>(function ToolbarButton(
  ...
  return (
    <IconButton color={active ? 'primary' : 'secondary'} disabled={disabled} onClick={onClick} ref={ref} {...props} >
      {children}
    </IconButton>
  );
});
